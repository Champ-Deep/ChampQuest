 ...
 * Each theme is a COMPLETE experience: unique font, companion persona,
 * motivational quotes, color palette, ranks, and sprite system.
 */
 ...
 * Returns { url, fallback, emoji, isEmoji }
 */
function getThemeSprite(rank) {
    const url = activeTheme.getSpriteUrl(rank.spriteId);
    if (url) {
        const fallback = activeTheme.getSpriteFallback ? activeTheme.getSpriteFallback(rank.spriteId) : null;
...
/**
 * Get a companion quote based on current level, rotating daily.
 */
function getCompanionQuote(level) {
    const quotes = activeTheme.companionQuotes;
    if (!quotes || quotes.length === 0) return '"Keep going, Champ!"';

    // Find the highest tier the user qualifies for
    let tier = quotes[0];
    for (let i = quotes.length - 1; i >= 0; i--) {
        if (level >= quotes[i].minLevel) {
            tier = quotes[i];
            break;
        }
    }
...
/**
 * Apply a theme by ID. Updates CSS variables, fonts, accent colors,
...
function applyTheme(themeId) {
    ...
    // Set data-theme attribute for CSS-level targeting
    document.body.setAttribute('data-theme', theme.id);

    // 1. Apply CSS custom properties (including --font-heading)
    Object.entries(theme.vars).forEach(([key, value]) => {
        root.style.setProperty(key, value);
    });

    // 2. Apply theme font to all .pixel-font elements
    document.querySelectorAll('.pixel-font').forEach(el => {
        el.style.fontFamily = theme.vars['--font-heading'];
    });

    // 3. Update mission scanner background dynamically
    ...
    // 4. Update team selector background gradient
    ...
    // 7. Update terminology labels via data attributes
    ...
    // 8. Update static text elements that reflect theme terminology
    ...
    // 12. Active missions section header
...
window.getCompanionQuote = getCompanionQuote;
